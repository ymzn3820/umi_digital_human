<template>
  <div class="main">
    ...
    <div class="right" style="width: 100%">
      <div class="go-back" style="margin-left: 10px">
      <div
        @click="goBackPage"
        :class="isShowSidbar ? 'go_back-item' : 'go_back-item-on'"
        v-if="page != 'myAudio'"
      >
        <img
                ...
                >
                  ...
                  <div class="pageOne-upload" v-if="!audio.new_url">
                    <img
                      ...
                      alt=""
                    />
                    <div class="upload-title">点击或拖拽音频到此处上传</div>
                    <div class="el-upload__text">
                      支持wav、mp3、ogg、m4a、aac、pcm格式
                    </div>
                    <div class="el-upload__text">文件小于20MB</div>
                  ...
                  <audio
                    v-else
                    :src="httpUrls.ossUrl + audio.new_url"
                    controls
                  ></audio>
                </el-upload>
              </div>
              ...
              <div class="audio-tips">
                <div class="audio-tips-title">
                  ...
                  <div
                    ...
                    @click="specificationBool = !specificationBool"
                  >
                    {{ !specificationBool ? "查看" : "收起" }}详细录音规范
                  </div>
                </div>
                    ...
                    <el-table-column prop="value" width="520" label="建议">
                      <template slot-scope="scope">
                        ...
                        <div v-if="scope.row.label == '录制环境'">
                          ...
                          <div>
                            2.避免远场环境下录制,建议离麦克风50cm之内,并需要保持位置、距离稳定,防止音量大小不一致。
                          </div>
                          <div>
                        ...
                        <div v-if="scope.row.label == '音频质量'">
                          <div>
                            1.必须仅含有一个人在说话,避免多个人同时说话。
                          </div>
                          <div>
                            2.说话人发音及音质越清晰、复刻质量越高。如说话人喉音/声音底噪严重,则复刻音频
                          </div>
                          <div>
                        ...
                        <div v-if="scope.row.label == '录制内容'">
                          <span
                            >建议在录音前熟悉文案,并确定好人设及演绎风格。尽量贴近复刻内容和主题,避免说话人韵律过于平淡,否则将得到较平的复刻效果。如需要复刻情绪韵律起伏较强的音频,请尽量输入表现力强的prompt。</span
                          >
                        </div>
                        <div v-if="scope.row.label == '音频格式'">
                          <span
                            >目前支持pcm、m4a、mp3、wav、ogg、aac、ogg_opus格式,当格式为pcm时,保证采样率为24k</span
                          >
                        </div>
                      </template>
                    </el-table-column>
      ...
      <div class="myAudio" v-if="page == 'myAudio'">
        ...
        <div class="myAudio-content">
          <div class="myAudio-content-top">
            <div>
              <div
                v-for="item in myAudioStatusArray"
                :style="
                  myAudioStatus == item.value
                    ? 'background: #FFF;color:#165DFF;'
                    : ''
                "
                @click="myAudioStatus = item.value"
                :key="item.value"
              >
                {{ item.label }}
              </div>
            ...
            <div>
              ...
              <el-button el-button type="primary" icon="el-icon-plus" style="padding: 6px 10px;margin-left: 20px;" @click="$router.push('/digitizedHuman/cloneAudio')">创建我的模型</el-button>
          ...
          <div class="myAudio-content-table">
            ...
            >
              ...
              <el-table-column prop="voice_status" label="状态">
                <template slot-scope="scope">
                  <div v-if="scope.row.voice_status == 1" style="color: #f00">
                    未付款
                  </div>
                  <div
                    v-if="scope.row.voice_status == 2"
                    style="color: #1f64ff"
                  >
                    训练中
                  </div>
                  <div
                    v-if="scope.row.voice_status == 3"
                    style="color: #00c777"
                  >
                    制作成功
                  </div>
                  <div v-if="scope.row.voice_status == 4" style="color: #f00">
                    制作失败
                  </div>
                  <div v-if="scope.row.voice_status == 5">启用</div>
                  <div v-if="scope.row.voice_status == 6" style="color: #f00">
                    过期
                  </div>
                  <div
                    v-if="scope.row.voice_status == 7"
                    style="color: #00c777"
                  >
                    付款成功
                  </div>
              ...
              <el-table-column prop="file_name" width="250" label="操作">
                <template slot-scope="scope">
                  <div class="scoped-box">
                    <audio
                      :ref="scope.row.voice_id"
                      v-if="
                        scope.row.voice_status == 3 ||
                        scope.row.voice_status == 5
                      "
                    ...
                    <div
                      v-if="
                        ...
                        scope.row.voice_status == 5
                      "
                      @click="playTableAudio(scope.row.voice_id)"
                    >
                      试听
                    </div>
                    <div
                    ...
                    <div
                      v-if="scope.row.voice_status == 5"
                      @click="(page = 'audioDetail'), (childPage = 'synthesis')"
                    >
                      合成声音
                    </div>
                  </div>
          ...
          <div>
            ...
            <div class="audioDetail-main-content">
              ...
              >
                <el-form-item
                  label="选择声音"
                  v-if="childPage == 'synthesis'"
                  prop="checkModel"
                >
                  <el-select
                    style="width: 100%"
                    v-model="ruleForm.checkModel"
                    placeholder="请选择"
                  >
                    <el-option
                      v-for="item in timbreData"
                      :value="item.voice_code"
            ...
            <div class="audioDetail-main-bottom">
              ...
              <div v-show="detailAudio.speech_url">
                <audio
                  :src="httpUrls.ossUrl + detailAudio.speech_url"
                  :ref="'detailAudio'"
                ></audio>
                ...
                <div style="margin-left: 20px">
                  <img
                    :src="
                      httpUrls.ossUrl +
                      'static/picture/e07e22bb-6d59-4c1a-a3c7-a5ad1d1e103d.png'
                    "
                    alt=""
                  />
            ...
            <div class="history-mask">
              ...
              >
                ...
                <div class="history-bottom">
                  <button class="domHover" @click.stop="download(item)">
                    下载
                  </button>
                  <button
                    class="domHover"
                    @click.stop="playAudio('audio' + item.h_code)"
                  >
                    播放
                  </button>
                  <button
                    class="domHover"
                    @click.stop="deleteVoice(item.h_code)"
                  >
                    删除
                  </button>
                </div>
          ...
          <div class="noHistory" v-if="false">
            <div>
              <img
                :src="
                  httpUrls.ossUrl +
                  'static/picture/86e840c2-e961-4649-848e-39d12c8c1cb0.png'
                "
                alt=""
              />
              <div>暂无记录</div>
      ...
      <div class="pay-success" v-if="page == 'pay'">
        ...
        <div class="pay-success-content">
          ...
          <img
            ...
            :src="
              httpUrls.ossUrl +
              'static/picture/01c48d79-77b4-4d5d-a5f9-c2e81f7971e0.png'
            "
          />
          <!-- 重新训练图片 -->
          <img
            ...
            :src="
              httpUrls.ossUrl +
              'static/picture/f7b00282-3d0c-4d61-8ee0-e925bcec7ec5.png'
            "
          />
          <div class="pay-text">
            {{ childPage == "pay" ? "支付成功" : "训练中" }}
          </div>
          <div class="pay-tips" v-if="childPage == 're-'">
            正在克隆，预计还剩时间10min
          </div>
          <div class="pay-time">
            {{ countdown }}m后跳转到{{
              childPage == "re-" ? "我的声音" : "声音克隆"
            }}页面
          </div>
    ...
    >
      <div>
        ...
        <div>
          请扫码前往<b>【无忧秘书智脑微信小程序】</b>完成认证授权后继续定制。
        </div>
        <img
          style="
            width: 160px;
...
export default {
  ...
  watch: {
    page: {
      deep: true, // 在对象内部的属性发生改变时，也能触发回调函数
      handler(newVal, oldVal) {
        this.oldPage = oldVal
      },
      immediate: true, // 添加 immediate 参数会立即以初始值的情况触发回调
    },
    myAudioStatus: {
      deep: true,
      handler() {
    ...
    dialogVisible: {
      deep: true,
      handler() {
        if (this.dialogVisible == true && this.dialogVisibleNum == 0) {
          //第一次开启校验
  ...
  created() {
    ...
    getEngine("scene_type=vg").then((res) => {
      ...
      getTimbre(`engine_code=${res.data[0]["engine_code"]}&voice_type=2`).then(
        (response) => {
  ...
  methods: {
    ...
    init() {
      let requestData = `voice_name=${this.searchValue}&voice_status=${
        this.myAudioStatus == 0 ? "" : this.myAudioStatus
      }`;
      //获取训练记录
      getAudioClone(requestData).then((res) => {
        this.audioData = res.data.data;
      ...
      await getCloneNumber().then((res) => {
        if (res.data.count_number != 0) {
          ...
          this.$message.success(
            `当前您还有${res.data}次声音克隆次数未使用，无需支付，请前往克隆`
          );
          this.page = "pageOne";
    ...
    handleDubbing(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          let speed_ratio = parseFloat(
            (this.ruleForm.speed * (3 - 0.2)) / 100 + 0.2
          ).toFixed(1); //语速
          let volume_ratio = parseFloat(
            (this.ruleForm.volume * (3 - 0.1)) / 100 + 0.1
          ).toFixed(1); //音量
          let pitch_ratio = parseFloat(
            (this.ruleForm.intonation * (3 - 0.1)) / 100 + 0.1
          ).toFixed(1); //语调
          let requestData = null;
          if (this.childPage == "synthesis") {
            //合成配音
            //合成配音--语音合成
            requestData = {
    ...
    re_(row) {
      this.$confirm(
        "你是否确认要重新训练？重新训练将会覆盖现有的训练结果，并且可能需要一段时间来完成。请在确认之前仔细考虑",
        "系统提示",
        {
          confirmButtonText: "确认",
        }
    ...
    firing(row) {
      this.$confirm(
        `启用后即可进行合成配音，但无法重新训练，是否确认启用${row.voice_name}`,
        "系统提示",
        {
          confirmButtonText: "确认",
        }
    ...
    handleDetail(row) {
      ...
      this.$set(
        this.detailAudio,
        "speech_url",
        JSON.parse(row.audios)[0].audio_url
      );
      this.page = "audioDetail";
    ...
    beforeAvatarUpload(file) {
      ...
      if (
        ...
        this.$message.error(
          "仅支持wav、mp3、ogg、m4a、aac、pcm类型的音频文件!"
        );
        return false;
      }
    },
